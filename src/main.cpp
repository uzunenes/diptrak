...
main(int argc, char** argv)
{
	...
	char video_stream_source_name[4096];
	char video_stream_output_name[4096];
	cv::VideoCapture cap;
	...
	while (1)
	{
		...
		detection_list = detect_objects(&dnnet, frame, app_debug);
		...
		print_detection_cv(detection_list);
