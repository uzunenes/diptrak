...
static void
create_new_tracks(std::vector<struct tracks>& tracks_objects, const std::vector<struct det_cv>& det_list, const std::vector<int>& unassigned_detections, bool kalman_filter_is_enable)
{
    int i;
    ...
    for (i = 0; i < (int)unassigned_detections.size(); ++i)
    {
        ...
        int index = unassigned_detections[i];

        new_tracks.id = g_next_id;
        new_tracks.bbox_cv = det_list[index].bbox_cv;
        new_tracks.obj_name = det_list[index].name;
...
delete_lost_tracks(std::vector<struct tracks>& tracks, int invisible_for_too_long_thresh, int age_thresh, float visibility_thresh)
{
    ...
    {
        float visibility = (float)tracks[i].total_visible_count / (float)tracks[i].age;
...
static void
update_unassigned_tracks(std::vector<struct tracks>& tracks_objects, const std::vector<int>& unassignments_tracks)
{
    int i;

    for (i = 0; i < (int)unassignments_tracks.size(); ++i)
    {
        int idx = unassignments_tracks[i];
...
static void
update_assigned_tracks(std::vector<struct tracks>& tracks_objects, const std::vector<struct det_cv>& det_list, const std::vector<assignment_idx>& assignments_indexs, bool kalman_filter_is_enable)
{
    int i;

    for (i = 0; i < (int)assignments_indexs.size(); ++i)
    {
        int track_idx = assignments_indexs[i].track_idx;
        int det_idx = assignments_indexs[i].det_idx;

        cv::Rect bbox_cv_det = det_list[det_idx].bbox_cv;

        if (kalman_filter_is_enable)
        {
            cv::Point det_center_point = get_center_bbox_cv(det_list[det_idx].bbox_cv);
...
static void
detection_to_track_assignment(const std::vector<struct tracks>& tracks_objects, const std::vector<struct det_cv>& det_list, std::vector<assignment_idx>& assignments_indexs, std::vector<int>& unassignments_tracks, std::vector<int>& unassigned_detections, int distance_th)
{
    int i, j;
    double cost;
    HungarianAlgorithm hung_algo;
    std::vector<int> assignment;
    std::vector<std::vector<double>> cost_matrix(det_list.size(), std::vector<double>(tracks_objects.size(), distance_th));
    std::vector<std::vector<int>> cost_matrix_det_idx(det_list.size(), std::vector<int>(tracks_objects.size(), distance_th));

    if (tracks_objects.size() == 0)
    {
        for (i = 0; i < (int)det_list.size(); ++i)
        {
    ...
    for (i = 0; i < (int)det_list.size(); ++i)
    {
        ...
        for (j = 0; j < (int)tracks_objects.size(); ++j)
        {
            ...
            cv::Point center_point2 = get_center_bbox_cv(det_list[i].bbox_cv);

            double distance = get_euclidean_distance_two_points(center_point1, center_point2);
...
update_tracks(std::vector<struct tracks>& tracks_objects, const std::vector<struct det_cv>& det_list, bool kalman_filter_is_enable, int distance_thresh_pixel, int invisible_for_too_long_thresh, int age_thresh, float visibility_thresh)
{
    ...
    detection_to_track_assignment(tracks_objects, det_list, assignments_indexs, unassignments_tracks, unassigned_detections, distance_thresh_pixel);

    update_assigned_tracks(tracks_objects, det_list, assignments_indexs, kalman_filter_is_enable);
    ...
    create_new_tracks(tracks_objects, det_list, unassigned_detections, kalman_filter_is_enable);
}
...
predict_new_locations_of_tracks(std::vector<struct tracks>& tracks_objects)
{
    ...
    for (i = 0; i < (int)tracks_objects.size(); ++i)
    {
        cv::Rect bbox_cv = tracks_objects[i].bbox_cv;

        update_dT(tracks_objects[i].kalman_filter, tracks_objects[i].last_tick_kf);
        cv::Point2f predicted_centroid = predict(tracks_objects[i].kalman_filter);
...
{
    int i;
    const int offset_pixl = 15;
    char buff_text[64];

    for (i = 0; i < (int)tracks_objects.size(); ++i)
    {
        draw_bbox_cv(tracks_objects[i].bbox_cv, m);
        cv::Point bbox_cv = cv::Point(tracks_objects[i].bbox_cv.x, tracks_objects[i].bbox_cv.y);

        cv::Point obj_name = cv::Point(bbox_cv.x, bbox_cv.y + offset_pixl);
        sprintf(buff_text, "%s", tracks_objects[i].obj_name.c_str());
        cv::putText(m, buff_text, obj_name, cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1.5, cv::LINE_AA);

        // put id
        cv::Point id_loc = cv::Point(bbox_cv.x, bbox_cv.y + 2 * offset_pixl);
        sprintf(buff_text, "id-%d", tracks_objects[i].id);
        cv::putText(m, buff_text, id_loc, cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1.5, cv::LINE_AA);

        // put age
        cv::Point age_loc = cv::Point(bbox_cv.x, bbox_cv.y + 3 * offset_pixl);
        sprintf(buff_text, "age-%d", tracks_objects[i].age);
        cv::putText(m, buff_text, age_loc, cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1.5, cv::LINE_AA);

        // put total visible count
        cv::Point t_vis_cnt = cv::Point(bbox_cv.x, bbox_cv.y + 4 * offset_pixl);
        sprintf(buff_text, "tvt-%d", tracks_objects[i].total_visible_count);
        cv::putText(m, buff_text, t_vis_cnt, cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1.5, cv::LINE_AA);

        cv::Point consecutive_invis_cnt = cv::Point(bbox_cv.x, bbox_cv.y + 5 * offset_pixl);
        sprintf(buff_text, "cit-%d", tracks_objects[i].consecutive_invisible_count);
        cv::putText(m, buff_text, consecutive_invis_cnt, cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar(0, 0, 255), 1.5, cv::LINE_AA);
    }
}
